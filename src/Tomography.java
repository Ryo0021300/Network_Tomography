...
public class Tomography {
    ...
    Tomography(ArrayList<Link> L){//コンストラクタ
        this.Links = new ArrayList<Link>(L);
        Initial_Path();
    }
...
//        ArrayList<ArrayList<Link>> Initial_Path = new ArrayList<ArrayList<Link>>();//各初期観測パスを格納
        ArrayList<Link> Tmp_Root = new ArrayList<Link>();//2次元リストの2次元目作成用の仮のリスト
        ArrayList<Link> Tmp_Decision = new ArrayList<Link>();
        ArrayList<Link> Selected = new ArrayList<Link>();//選択済みリンク
        ArrayList<Link> Not_Selected;//未選択リンク

        Not_Selected = new ArrayList<Link>(this.Links);//まず全リンクを未選択リストに入れる
